...
GraphBounds calculateGraphBounds(const LinkedList* points) {
    ...
    bounds.min.x = firstPoint->x;
    bounds.max.x = firstPoint->x;
    bounds.min.y = firstPoint->y;
    bounds.max.y = firstPoint->y;

    while (hasNext(&it)) {
        GraphPoint* point = (GraphPoint*)get(&it);
        bounds.min.x = getMinDouble(point->x, bounds.min.x);
        bounds.max.x = getMaxDouble(point->x, bounds.max.x);
        bounds.min.y = getMinDouble(point->y, bounds.min.y);
        bounds.max.y = getMaxDouble(point->y, bounds.max.y);
        next(&it);
    }
    return bounds;
}
...
void drawMetricLine(DrawContext* ctx, double value, QColor color, QString label) {
    int yCoord = mapToY(value, ctx->bounds.min.y, ctx->bounds.max.y, ctx->height);
...
void drawMetricDecorations(DrawContext* ctx) {
    ctx->painter->setPen(QColor("#00FF00"));
    ctx->painter->drawText(-boundsLabelXOffset, boundsLabelYOffset, QString::number(ctx->bounds.max.y, 'f', 2));
    ctx->painter->drawText(-boundsLabelXOffset, ctx->height + boundsLabelYOffset, QString::number(ctx->bounds.min.y, 'f', 2));
...
void adjustGraphValueBounds(GraphBounds* bounds) {
    if (isEqual(bounds->min.y, bounds->max.y)) {
        bounds->min.y -= FALLBACK_VALUE_OFFSET;
        bounds->max.y += FALLBACK_VALUE_OFFSET;
    }

    bounds->min.y = roundDownToStep(bounds->min.y, VALUE_STEP) - VALUE_STEP;
    bounds->max.y = roundUpToStep(bounds->max.y, VALUE_STEP) + VALUE_STEP;
}

void adjustGraphYearBounds(GraphBounds* bounds) {
    if (isEqual(bounds->min.x, bounds->max.x)) {
        bounds->min.x -= 1.0;
        bounds->max.x += 1.0;
    } else {
        int offset = getMaxInt(1, (int)((bounds->max.x - bounds->min.x) * 0.05));
        bounds->min.x -= offset;
        bounds->max.x += offset;
    }
}
...
QList<QLine> buildLines(const LinkedList* points, DrawContext* ctx) {
    ...
    if (!isEmpty(points)) {
        ...
        while (hasNext(&it)) {
            ...
            int x = mapToX((int)p->x, (int)ctx->bounds.min.x, (int)ctx->bounds.max.x, ctx->width);
            int y = mapToY(p->y, ctx->bounds.min.y, ctx->bounds.max.y, ctx->height);
